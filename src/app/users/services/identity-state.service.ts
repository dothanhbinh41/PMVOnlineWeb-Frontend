...
export class IdentityStateService {
  ...
  getDepartments() {
    return this.store.selectSnapshot(IdentityState.getUserDepartments);
  }
  getRoles() {
  ...
  dispatchGetUserDepartments(...args: ConstructorParameters<typeof GetUserDepartments>) {
    return this.store.dispatch(new GetUserDepartments(...args));
  }
}
